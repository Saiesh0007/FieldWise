...
| **Projects System & Autosave** | Local-first project management (create, rename, delete, recency sort) with debounced IndexedDB persistence. Every app launch starts from a blank "Untitled Project," never auto-resuming whatever was last open — reopening a specific saved project is a deliberate action from the Projects panel. | `src/lib/storage/project.ts`, `projectDb.ts`, `useProjectAutosave.ts` |
...
| **Flight Path Preview** | A blue arrow drone marker animates the actual planned route, Start to Finish, over the same yellow/blue plan lines and S/F markers shown on Plan — auto-plays on a fixed-duration preview clock (not real drone speed). Simulate-scoped Play/Pause, Brake (pause in place), and RTL (reset to Start) controls, independent of Send to Vehicle's identically-named live flight controls. | `src/lib/simulation/flightPreview.ts`, `SimulatePanel.tsx`, `FieldMap.tsx`'s `dronePosition` marker |
...
* **Code Quality & Testing:** Vitest (27 suites, 230 tests passing), Oxlint.
