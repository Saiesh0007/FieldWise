...
* **Key Deliverables:**
  ...
  * **Start & Finish Points:** the mission's actual flight-path start (first pass's start) and finish (last pass's end) — not the same as the boundary's first vertex — render as matching green DOM markers labeled "S"/"F" (`FieldMap.tsx`'s `planStartPoint`/`planFinishPoint` markers via `maplibre-gl`'s `Marker`, not a GeoJSON symbol layer — this project's map style has no glyphs server, deliberately, so text-field labels wouldn't render). Automatically nudged apart when the route starts and ends within 12m of itself (a real, common boustrophedon outcome — the sample field does this), so neither marker ever silently sits underneath the other. Paired with live mission-progress tracking in Send to Vehicle: `MISSION_ITEM_REACHED` (already a registered message, previously undecoded) now drives a progress bar and a "Finish point reached — mission 100% complete" banner once the vehicle reports reaching the last uploaded waypoint.
