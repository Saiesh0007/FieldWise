...
* **Key Deliverables:**
  ...
  * **Four Plot-Creation Methods:** RC/Mobile (GPS walk), Drone (click-to-place at each corner, or an auto-play "Simulate demo"), Map (satellite trace), and Import KML/GeoJSON, behind a 2×2 method selector on the Import panel (`ImportPanel.tsx`, `DronePointCapture.tsx`) — completing AeroGCS Green's four documented plot-creation methods. The created plot renders with a blue fill (`boundary-fill-layer`, `#2563eb`) and a yellow outline (`boundary-plain-outline-layer`, `#eab308`) on every step except Verify, where the per-edge provenance colors take over instead.
  ...
  * **Flight Path Preview:** A blue arrow marker (`FieldMap.tsx`'s `dronePosition` prop, a `maplibre-gl` `Marker` rotated by `Marker.setRotation`) flies the actual planned route Start to Finish on the Simulate step, on a fixed ~18s preview clock decoupled from the drone profile's real `speedMps` (a multi-kilometer plan at real speed would take the better part of an hour). Play/Pause, Brake (pause in place), and RTL (reset to Start) are Simulate-scoped — separate component state from Send to Vehicle's identically-named live flight-control buttons, usable without ever connecting a vehicle (`SimulatePanel.tsx`, `src/lib/simulation/flightPreview.ts`'s pure `buildFlightPath`/`poseAtDistance`).
  * **Manual Plan Editing (§11, partial):** **Adjust Spacing** overrides the profile-derived row spacing (2–10m slider); **Route Adjust** gets an angle slider alongside the existing fixed-heading input, plus a Head Lock toggle (recorded, doesn't change geometry — FieldWise has no in-flight heading simulation to lock); **Move Plan** nudges the generated plan by an accumulated local-meter offset via directional buttons, independent of the boundary/zones; **Plan Splitting** marks N% of the route (by pass count) as this sortie's included portion, in any of three directions — from start, from end, or both sides at once (the included percentage split evenly across both ends, excluding the single middle chunk) — matching AeroGCS Green's own coloring: intended route yellow, deferred portion a wide semi-transparent blue overlay. Upload mission sends only the included subset when a split is active. Reset back to 100% (`PlanPanel.tsx`, `useFieldStore.ts`'s `spacingOverrideM`/`headLock`/`planOffsetLocal`/`planSplitPercent`/`planSplitDirection`, `planner.ts`'s `translateSprayPlan`/`splitPlanPasses`, `missionFromPlan.ts`).
  * **Start & Finish Points:** the mission's actual spray-route start (first *spraying* pass's start) and finish (last *spraying* pass's end) — deliberately skipping the transit legs every sortie is bookended with out from and back to the home/refill point, so Start and Finish are never the same point the way they would be reading the literal first/last pass — render as matching green DOM markers labeled "S"/"F" (`FieldMap.tsx`'s `planStartPoint`/`planFinishPoint` markers via `maplibre-gl`'s `Marker`, not a GeoJSON symbol layer — this project's map style has no glyphs server, deliberately, so text-field labels wouldn't render). A 12m same-side nudge remains as a defensive fallback for pathological geometry, but is no longer load-bearing (see Memory.md ADR-018). Paired with live mission-progress tracking in Send to Vehicle: `MISSION_ITEM_REACHED` (already a registered message, previously undecoded) now drives a progress bar and a "Finish point reached — mission 100% complete" banner once the vehicle reports reaching the last uploaded waypoint. The uploaded mission itself is trimmed of those same bookending transit legs (`missionFromPlan.ts`), so it no longer auto-flies home at the end — that's now RTL's job, a dedicated flight-control button alongside Brake/Resume/Land.

> **A Serial↔WebSocket bridge is now built** (2026-09-23) — `bridge/serial-ws-bridge.mjs` + `src/lib/vehicle/piRelayVehicle.ts` — for the case this project's own hardware actually needed: a Pixhawk wired to a Raspberry Pi with no telemetry radio, reachable only over SSH, where Web Serial can't reach the Pixhawk at all since the browser isn't on the same machine as the serial port. See Memory.md ADR-014 and `bridge/README.md`.
>
> **Still not started:** a TCP↔WebSocket bridge specifically to let FieldWise connect to Mission Planner's *SITL* simulator (a different use case — relaying a local TCP socket, not a real serial port) for hardware-free testing. The same `PiRelayVehicle`/bridge pattern above could likely be adapted for this with a small serial-vs-TCP swap in the bridge script, but that adaptation hasn't been done.
>
...
>
> **§11 Manual Plan editing — mostly built.** Route Adjust (angle slider + Head Lock), Adjust Spacing, Move Plan, and Plan Splitting are done (see below), and Plan Splitting now also filters what "Upload mission" actually sends (see Memory.md ADR-016's update). Still missing: **Adjust Waypoint** (drag/nudge an individual generated waypoint — architecturally the hard one, since the plan is always regenerated fresh from the boundary/profile/strategy on every `recompute()`; a manually-dragged waypoint would need a real "manual override that survives the next re-plan" design, not just a one-off edit), **Indentation** (per-edge inward buffer from the boundary), and **Obstacle Boundary spacing** (buffer distance around obstacles).
