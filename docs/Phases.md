...
* **Key Deliverables:**
  ...
  * **Manual Plan Editing (§11, partial):** **Adjust Spacing** overrides the profile-derived row spacing (2–10m slider); **Route Adjust** gets an angle slider alongside the existing fixed-heading input, plus a Head Lock toggle (recorded, doesn't change geometry — FieldWise has no in-flight heading simulation to lock); **Move Plan** nudges the generated plan by an accumulated local-meter offset via directional buttons, independent of the boundary/zones; **Plan Splitting** marks the first/last N% of the route (by pass count, chosen direction) as this sortie's included portion — drawn blue on the map for the excluded rest — to help a pilot optimize battery usage across multiple sorties, with a Reset back to 100% (`PlanPanel.tsx`, `useFieldStore.ts`'s `spacingOverrideM`/`headLock`/`planOffsetLocal`/`planSplitPercent`/`planSplitFromEnd`, `planner.ts`'s `translateSprayPlan`/`splitPlanPasses`).
  * **Start & Finish Points:** the mission's actual flight-path start (first pass's start) and finish (last pass's end) — not the same as the boundary's first vertex — render as distinct green/red map markers (`planner.ts`'s `planStartPoint`/`planFinishPoint`, `FieldMap.tsx`). Paired with live mission-progress tracking in Send to Vehicle: `MISSION_ITEM_REACHED` (already a registered message, previously undecoded) now drives a progress bar and a "Finish point reached — mission 100% complete" banner once the vehicle reports reaching the last uploaded waypoint.
...
>
> **§11 Manual Plan editing — mostly built.** Route Adjust (angle slider + Head Lock), Adjust Spacing, Move Plan, and Plan Splitting are done (see below). Still missing: **Adjust Waypoint** (drag/nudge an individual generated waypoint — architecturally the hard one, since the plan is always regenerated fresh from the boundary/profile/strategy on every `recompute()`; a manually-dragged waypoint would need a real "manual override that survives the next re-plan" design, not just a one-off edit), **Indentation** (per-edge inward buffer from the boundary), and **Obstacle Boundary spacing** (buffer distance around obstacles). Plan Splitting's built version doesn't yet upload only the included subset (Upload mission still sends the whole plan) — the split is a planning/visualization aid and a real Resume-mid-mission control (via the flight controls' Resume button, not a Plan-Splitting-specific one), not (yet) a mission-upload filter.
