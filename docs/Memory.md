...
* **Superseded in part by ADR-020** (2026-09-24): the "resume the remembered active project on every launch" half of this decision was reversed — every fresh app start now begins from a blank "Untitled Project" regardless of what was last active. The IndexedDB storage, the autosave hook, and `localStorage`'s remembered active-project id described above are all still exactly as decided here; only the automatic resume-on-boot behavior changed.
...
### ADR-020: Reversed ADR-006 — the App No Longer Resumes the Last Project on Launch
* **Date:** 2026-09-24
* **Status:** Accepted & Implemented
* **Context:** "when I start it should show an untitled project but it shows a project name that already exists when i start the software." ADR-006's original design was deliberate: `useProjectAutosave`'s mount effect read the remembered active project id from `localStorage` and, on every app launch, resolved and loaded that project's full snapshot into the session — so refreshing mid-edit (or just reopening the app tomorrow) picked up exactly where the pilot left off, framed at the time as matching "how the rest of FieldWise already works." In practice this meant the header's project name, and the entire loaded boundary/plan, came back from whatever was last active — which reads as "the software is showing me someone else's project" the moment that name isn't the one the pilot expects to see on a fresh launch, exactly what was reported.
* **Decision:** Removed the mount-time resume effect from `useProjectAutosave.ts` entirely (along with the `resumedRef` guard that existed only to sequence autosave after it). Every app launch now starts from the store's own default state — no active project, no boundary, `activeProjectName` at its default `'Untitled Project'` — regardless of what `localStorage`'s remembered id or IndexedDB's saved records say. That remembered id and the saved records aren't deleted or made otherwise inert: `ProjectsPanel.tsx`'s `openProject` (click a project in the list) and "+ New Project" flows still explicitly call `setActiveProjectId`/`loadProjectSnapshot` themselves, unaffected by this change — reopening a specific past project is now something the pilot does on purpose from the Projects panel, never something that happens to them automatically on launch. The lazy "Untitled Project" autosave-on-first-edit behavior (ADR-006's other half) is untouched — drawing/loading a boundary in a fresh session still creates and autosaves a new project the same as before.
* **Consequences:**
...
### ADR-021: Mission Item Seq 0 Is HOME, Not a Waypoint — First Real Hardware Bug Found
* **Date:** 2026-09-24
* **Status:** Accepted & Implemented
* **Context:** First upload against a real Pixhawk 2.4.8 (wired to a Raspberry Pi 4B over TELEM2, no USB/Web Serial involved — see the Hardware Gotchas entry below for the connection setup itself). The mission upload/download round trip completed without error — no timeout, no rejection — but came back with `"1 mismatch(es) — Position off by (3.04e+1, 7.58e+1) deg."` The offset (~30.4°, ~75.8°) was suspiciously close to the field's own real coordinates (~30.35° lat, ~75.75° lon), which pointed at the read-back value being close to **(0, 0)**, not corrupted data. ArduPilot's mission protocol reserves item **seq 0** for the vehicle's HOME position — Mission Planner and QGroundControl always insert a dedicated home item there and start the real route at seq 1; this project never did that, sending the route's own first waypoint as seq 0 instead. ArduPilot silently substitutes its own home position for whatever's uploaded at seq 0 (here, still unset since the vehicle was being tested indoors with no GPS fix — hence effectively (0, 0)), so the read-back for that one item never matched what was sent. This is exactly the category of failure `VEHICLE_CONNECTION_CHECKLIST.md` flagged in advance ("something in the mission-protocol handshake behaving differently than the simulated vehicle... assumed") — `mavlinkSession.test.ts`'s `FakeVehicle` faithfully echoes back whatever it's sent, so it had no way to catch a real-firmware behavior it never modeled.
* **Decision:** `MavlinkSession.uploadAndVerifyMission` now takes a second parameter, `homePosition: LatLng`, and prepends it as a synthetic mission item at wire seq 0, shifting every real waypoint's wire seq by +1 — `waypoints[i].seq` (0-indexed, as `sprayPlanToWaypoints` already produces) becomes wire seq `i + 1`. Verification skips wire seq 0 entirely rather than comparing it — the vehicle owns that slot and is expected to report back its own home position, not what was sent, so treating a mismatch there as a failure would be flagging correct behavior as broken. `uploadedCount` in the result still reports the real route's length, not the +1 wire total, so the UI's language ("Sends the current plan's N legs...") didn't need to change. The empty-mission case (Plan Splitting at 0%) short-circuits before any of this — no home item is sent either, matching the existing "nothing to upload" no-op. `SendPanel.tsx` derives `homePosition` the same way `splitIntoSorties` (droneProfile.ts) derives its own home point — `sprayPlan.sorties[0].passes[0].start`, i.e. the actual first pass in flight order before `trimBookendingTransitLegs` removes it (ADR-018) — rather than `boundary.vertices[0]` directly, so a Move Plan offset (which shifts every pass, including that one) is correctly reflected; falls back to `boundary.vertices[0]` only if `sorties` is somehow empty. The live mission-progress bar (`telemetry.lastReachedWaypointSeq`, itself a wire seq from `MISSION_ITEM_REACHED`) was updated to match: it now directly counts "real legs completed" (0 = still at home, `total` = last real leg reached) without the `+1`/`-1` indexing the old scheme needed.
* **Consequences:**
  * *Pros:* Found and fixed on the very first real-hardware upload attempt, exactly the kind of gap the project's own checklist predicted unit tests couldn't catch. 3 new/updated tests in `mavlinkSession.test.ts` — one exercises the exact regression via a `FakeVehicle.overwriteHomeWithZero` option that simulates ArduPilot's real substitution behavior, proving the fix genuinely ignores a wrong-looking seq-0 read-back instead of merely getting lucky — 230 tests total now. Every layer between `sprayPlanToWaypoints` (unaware of any of this, still numbers the real route 0..N-1) and `mavlinkSession.ts` (the only place that knows ArduPilot's seq-0 convention) stays cleanly separated — `MissionWaypoint`/`SprayPass` types didn't need to change at all.
  * *Cons:* Still not verified whether the *count* semantics (whether `MISSION_ITEM_REACHED` for wire seq 0 itself ever gets reported by real firmware, e.g. immediately after arming) behaves as assumed — this fix was verified via the mission upload/verify path on real hardware, not yet via a full in-flight run reporting live progress against the shifted seq numbering. `homePosition` is currently always the plan's own home/refill point, never the vehicle's actual current GPS position at upload time (which is what Mission Planner typically uses) — functionally fine, since ArduPilot overwrites it either way once it has its own fix, but worth knowing if a future report expects the *uploaded* home value itself to reflect where the vehicle is actually standing.
...
6. **Mission item seq 0 is HOME, not a waypoint** (see ADR-021): ArduPilot's mission protocol always treats the item at seq 0 specially and substitutes its own home position for whatever's uploaded there — a real GCS (Mission Planner, QGroundControl) always sends a dedicated home item first and starts the actual route at seq 1. Skipping this and sending a real waypoint as seq 0 doesn't fail the upload — it silently reads back wrong on that one item, which without a GPS fix yet can look like the position was corrupted to something near (0, 0).

### Raspberry Pi ↔ Pixhawk via TELEM2 (UART, not USB)
A companion computer wired to the Pixhawk's TELEM2 port (rather than a Pixhawk connected to the Pi over USB) is a genuinely different transport, not just a different cable:
* It's the Pi's hardware UART (GPIO pins 8=TXD/10=RXD/6=GND on the 40-pin header, wired **crossed** to TELEM2's TX/RX/GND — don't connect TELEM2's 5V pin unless deliberately powering the Pi from it), not a USB-serial adapter — so it will never show up in `lsusb` or `/dev/serial/by-id/`, and troubleshooting a missing device there is the wrong direction entirely.
* The Pi's serial port has to be explicitly enabled first (`raspi-config` → Interface Options → Serial Port, or `enable_uart=1` in `/boot/config.txt`/`/boot/firmware/config.txt`) — on Pi models with onboard Bluetooth, the primary UART defaults to being shared with the Bluetooth module or the login console instead of being free for this. Once enabled, `/dev/serial0` symlinks to the real device (`ttyAMA0` — the full PL011 UART, ideal — or `ttyS0`, the lighter-weight mini-UART, depending on the Pi model/config).
* TELEM2's baud rate has to match what the bridge is told to use — ArduCopter's stock default for TELEM2 is **57600**, distinct from the 115200 a direct USB connection typically uses. Worth checking `SERIAL2_BAUD` in Mission Planner/QGroundControl if it's ever been configured differently on a given board.
* `bridge/serial-ws-bridge.mjs --serial /dev/serial0 --baud 57600 --port 8765` is otherwise identical to the USB case from FieldWise's side — same "Pi bridge" connection mode, same `ws://<pi-ip>:8765` address.
...
* **Status:** 27 test files, 230 tests passing (100% success rate).
* **Key Test Suites:**
