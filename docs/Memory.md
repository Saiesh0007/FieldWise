...
* **Consequences:**
  * *Pros:* 8 new tests (`flightPreview.test.ts`) — 225 total now. Browser-verified: the blue arrow renders and visibly moves/rotates across two screenshots 4 seconds apart (302m → 2967m of 12931m flown); exactly one `maplibregl-marker` exists on Simulate (the drone; S/F don't render there since `sprayPlan`'s own map prop is nulled on that step, same as `boundary`); Brake/RTL text is present on Simulate and absent on Plan; `boundary-fill-layer`'s paint and `boundary-plain-outline-layer`'s color/visibility read back exactly as set via `map.getPaintProperty`/`getLayoutProperty`.
